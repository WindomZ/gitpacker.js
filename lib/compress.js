 ...
 * @api public
 */
function _compress (method = '', dir = process.cwd(), file = process.cwd(), includes = [], excludes = []) {
  if (!includes) {
    includes = []
...
function excludeFileWalker (dir) {
  ...
  dirList.forEach(item => {
    ...
    if (stat.isFile()) {
      ...
      if (exclude(item) || assetExclude(item, true)) fileList.push(path.relative(process.cwd(), file))
    }
  })
...
function * compress (method, dir, file, includes, excludes) {
  return _compress(method, dir, file, includes, excludes)
}
...
module.exports.compress = _compress
